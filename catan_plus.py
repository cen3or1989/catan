...
class Game:
    ...
    def player_connected_nodes(self, player: Player) -> Set[int]:
        ...
        # Return only nodes that are connected AND not occupied by opponents
        return (seen | owned_nodes) - opponent_occupied

    def legal_road_spots(self, player: Player) -> List[Tuple[int,int]]:
        blocked = self.all_roads()
        
        # Get connected nodes (now properly excludes opponent-occupied nodes)
        connected = self.player_connected_nodes(player)
        
        # Nodes that are occupied by OPPONENTS
        opponent_occupied = set()
        for p in self.players:
            if p.id == player.id:
                continue
        ...
        for e in self.board.edges:
            ...
            # CRITICAL: Both endpoints must be available (not occupied by opponents)
            # This prevents building roads through opponent settlements/cities
